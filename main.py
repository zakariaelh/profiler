...
def generate_branch_name():
    ...
    return f"profiler-{random_suffix}"
...
def generate_pull_request(branch_name: str, owner: str, repo: str):
    repository = github_client.get_repo(f"{owner}/{repo}")

    # Create the pull request
    pr = repository.create_pull(
        title="Optimization by Profiler",
        body="This pull request contains optimizations generated by the profiler.",
        head=branch_name,
        base="main"
    )
...
def main(owner, repo, pr_number):
    ...
    if changes_made:
        ...
        pr_url = generate_pull_request(branch_name, owner=owner, repo=repo)
